...
def showdata():
    ...
    # cursor = conn.cursor()
    # cursor.execute(
    #      "SELECT date_time,household_key,product_chnm,product_price,total_qty \
    #      FROM PStage.MemberImmediateTaking_KSQL_History \
    #      WHERE MemberImmediateTaking_KSQL_History.household_key IN \
    #      (SELECT line_smart.household_key FROM PStage.line_smart \
    #      WHERE line_smart.user_id = '{}');".format(checkuserid))
...
def showcar():
    ...
    # cursor = conn.cursor()
    # sql_str = "SELECT \
    # STR_TO_DATE(trans_date,'%Y%m%d%'),\
    # product_chnm,\
    # ROUND(total_qty,0) AS totalnumber,\
    # product_price,\
    # ROUND(total_price,0) \
    # FROM PStage.MemberImmediateTaking_KSQL \
    # WHERE MemberImmediateTaking_KSQL.household_key \
    # IN (SELECT line_smart.household_key FROM PStage.line_smart WHERE line_smart.user_id = '{}');".format(checkuserid)
    # cursor.execute(sql_str)
    # data = cursor.fetchall()
...
def recommendation():
    ...
    # WHERE product_recommendinfo.product_name_A \
    # IN (SELECT product_chnm FROM PStage.MemberImmediateTaking_KSQL WHERE MemberImmediateTaking_KSQL.household_key \
    # IN (SELECT line_smart.household_key FROM PStage.line_smart WHERE line_smart.user_id ='{}'));".format(checkuserid)
    # cursor.execute(sql_str)
...
def personal_recommend():
    ...
    #           + "WHERE PStage.Diamond_Recommend_Result.USERID " \
    #           + "IN (SELECT line_smart.household_key "\
    #           + "FROM PStage.line_smart "\
    #           + "WHERE line_smart.user_id = '{}' AND Season ='{}');".format(checkuserid, season)
    # conn = pymysql.connect()
    # cursor = conn.cursor()
    # cursor.execute(sql_str)
    # data = cursor.fetchall()
...
def show_user_info(line_uid):
    # mydb = pymysql.connect()
    # cursor = mydb.cursor()
    # cursor.execute("SELECT account, password FROM line_smart WHERE user_id = '{}';".format(user_id))
    # result = cursor.fetchall()
...
# def member_user_housekey(user_id):
#     mydb = pymysql.connect()
#     cursor = mydb.cursor()
#     result = cursor.execute("select household_key FROM line_smart where user_id = '{}';".format(user_id))
#     return result
...
def process_text_message(event):
    ...
    profile_dic = vars(profile)
    # displayname = profile_dic.get('display_name')
    # line_uid = profile_dic.get('user_id')
    # household_key = uid_to_household_key(line_uid)
